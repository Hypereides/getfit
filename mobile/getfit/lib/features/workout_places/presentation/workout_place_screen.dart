  ...
  Widget _buildBody() {
    switch (_state) {
      ...
      case _LoadState.loaded:
        final r = _result!;
        if (r.open.isEmpty && r.unknownHours.isEmpty) {
          return _NoAvailableWorkoutPlacesScreen(
            onMessageRead: () => Navigator.pop(context),
          );
        }
        return _AvailableWorkoutPlacesScreen(
          result: r,
...
class _AvailableWorkoutPlacesScreen extends StatefulWidget {
  const _AvailableWorkoutPlacesScreen({
    required this.result,
    required this.onRefresh,
  });

  final WorkoutPlaceResult result;
  final VoidCallback onRefresh;

  @override
  State<_AvailableWorkoutPlacesScreen> createState() =>
      _AvailableWorkoutPlacesScreenState();
}

class _AvailableWorkoutPlacesScreenState
    extends State<_AvailableWorkoutPlacesScreen> {
  ...
  void _showWorkoutPlaceInfo(WorkoutPlace place) {
    showModalBottomSheet(
      ...
      builder: (_) => _WorkoutPlaceInfoScreen(
        place: place,
        onCancel: () => Navigator.pop(context),
        onDirections: () {
          Navigator.pop(context);
          _WorkoutPlaceInfoScreen.openDirections(context, place);
        },
      ),
    );
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (r.unknownHours.isNotEmpty) ...[
        ...
        _expandable(
          ...
          onTap: () =>
              setState(() => _unknownExpanded = !_unknownExpanded),
        ),
        if (_unknownExpanded) ...[
      ...
      if (r.closed.isNotEmpty) ...[
        ...
        _expandable(
          ...
          onTap: () =>
              setState(() => _closedExpanded = !_closedExpanded),
        ),
        if (_closedExpanded) ...[
          const SizedBox(height: 12),
...
class _NoAvailableWorkoutPlacesScreen extends StatelessWidget {
  const _NoAvailableWorkoutPlacesScreen({required this.onMessageRead});
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.access_time_rounded,
                size: 72, color: Colors.orange[300]),
            const SizedBox(height: 24),
            const Text(
              'No Places Open Now',
...
class _WorkoutPlaceInfoScreen extends StatelessWidget {
  const _WorkoutPlaceInfoScreen({
    required this.place,
  ...
  static Future<void> openDirections(
      BuildContext context, WorkoutPlace place) async {
    final uri = Uri.parse(
            ...
            child: ListView(
              ...
              children: [
                ...
                _row(Icons.schedule_outlined, 'Hours',
                    place.availableHours),     // getAvailableHours()
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isOpen == true ? Colors.white : Colors.grey[50]!,
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
